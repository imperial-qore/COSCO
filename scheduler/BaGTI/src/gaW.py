...
class Individual(object): 
    ...
    def mate(self, par2): 
        ...
        return Individual(self.dataset, self.apps, self.graph, self.f, self.bounds, self.data_type, self.hosts, child_chromosome) 
  
    def cal_fitness(self): 
        res = self.f(self.chromosome, self.apps, self.graph)
        return res
  
def gaW(dataset, apps, graph, f, bounds, data_type, hosts): 
    generation = 1
    population = [] 
    best_fitness = []
  
    for _ in range(POPULATION_SIZE): 
        population.append(Individual(dataset, apps, graph, f, bounds, data_type, hosts)) 
